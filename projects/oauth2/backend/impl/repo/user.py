...
class AuthenticationPO(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_uuid = Column(UUID(as_uuid=True), ForeignKey("oauth2.app_user.uuid"), nullable=False)

    provider = Column(String(20), nullable=False)
    provider_id = Column(String(20), nullable=False)

    credentials = Column(JSONB)  # json
...
class UserRepo:
    ...
    async def create_authentication(
        self, user_uuid: uuid.UUID, provider: str, provider_id: str, credentials: dict
    ) -> Authentication:
