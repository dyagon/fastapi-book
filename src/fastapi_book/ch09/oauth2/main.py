...
from .db import fake_users_db, fake_client_db, pwd_context
from .models import UserInDB, Client, Token
...
async def authorize_get(
    ...
    if response_type != "code":
        ...
        return RedirectResponse(
            url=f"{redirect_uri}?{urllib.parse.urlencode(error_params)}",
            status_code=302
        )
    ...
    print(redirect_uri, client.redirect_uris)
...
async def token_endpoint(
    ...
    client_id_auth, client_secret_auth = get_client_credentials(
        credentials, client_id, client_secret
    )
    print("Client ID Auth:", client_id_auth)
    ...
    if grant_type == "authorization_code":
        ...
        return handle_authorization_code_grant(
            code, redirect_uri, client_id_auth, client_secret_auth, code_verifier
        )
        ...
        return handle_refresh_token_grant(
            refresh_token, client_id_auth, client_secret_auth
        )
    
    elif grant_type == "client_credentials":
        return handle_client_credentials_grant(
            client_id_auth, client_secret_auth, scope
        )
